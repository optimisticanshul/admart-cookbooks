...
if node['cpu'].nil? || node['cpu']['total'].nil?
  ...
  default['mysql']['tunable']['innodb_thread_concurrency']       = (Integer(node['cpu']['total']) * 2).to_s
  default['mysql']['tunable']['innodb_commit_concurrency']       = (Integer(node['cpu']['total']) * 2).to_s
  default['mysql']['tunable']['innodb_read_io_threads']          = (Integer(node['cpu']['total']) * 2).to_s
end
